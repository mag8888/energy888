import { Room, Player, CreateRoomData, JoinRoomData, SelectProfessionData, ConfirmProfessionData } from '../types';
import { RoomUtils } from '../utils/roomUtils';
import { ProfessionUtils } from '../utils/professions';
...
export class RoomController {
   ...
   * Создать комнату
   */
  async createRoom(data: CreateRoomData): Promise<{ success: boolean; room?: Room; error?: string }> {
    try {
      // Валидация данных
   ...
   */
  async joinRoom(data: JoinRoomData, socketId: string): Promise<{ success: boolean; room?: Room; error?: string }> {
    try {
      // Валидация данных
   ...
   * Покинуть комнату
   */
  async leaveRoom(roomId: string, socketId: string): Promise<{ success: boolean; room?: Room; error?: string }> {
    try {
   ...
   * Выбрать профессию
   */
  async selectProfession(data: SelectProfessionData): Promise<{ success: boolean; error?: string }> {
    try {
      const room = await this.getRoom(data.roomId);
      if (!room) {
        return { success: false, error: 'Комната не найдена' };
   ...
   */
  async confirmProfession(data: ConfirmProfessionData): Promise<{ success: boolean; error?: string }> {
    try {
      const room = await this.getRoom(data.roomId);
      if (!room) {
        return { success: false, error: 'Комната не найдена' };
   ...
   */
  async getAvailableProfessions(roomId: string): Promise<{ 
    success: boolean; 
    professions?: string[]; 
   ...
   */
  isRoomReadyToStart(room: Room): boolean {
   ...
   */
  shouldDeleteRoom(room: Room): boolean {
